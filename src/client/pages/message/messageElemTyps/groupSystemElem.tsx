...
export const GroupSysElm = (props: any): JSX.Element => {
    ...
    const opType = ["", "有人申请加群", "申请加群被同意", "申请加群被拒绝", "被管理移出群", "群被解散", "创建群", "您被邀请加群", "主动退群", "被设置管理员", "被取消管理员", "群已被回收", "邀请加群", "邀请加群被同意", "邀请加群被拒绝", "已读上报多终端同步通知", "用户自定义通知"]
    const getOpUser = () => {

    }
    const getOpType = () => {
        return props.group_report_elem_op_user_info.user_profile_nick_name + opType[props.group_report_elem_report_type]
    }
    const item = (props) => {
        return (
