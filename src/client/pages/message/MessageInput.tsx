...
const FEATURE_LIST = {
  1: FEATURE_LIST_C2C,
  2: FEATURE_LIST_GROUP,
};
export const MessageInput = (props: Props): JSX.Element => {
  const { convId, convType, isShutUpAll, handleOpenCallWindow, isHandCal } =
    props;
  const [isDraging, setDraging] = useState(false);
  const [activeFeature, setActiveFeature] = useState("");
  const [shouldShowCallMenu, setShowCallMenu] = useState(false);
  //解决打开文件无法发送问题
  const [sendMessageFile, setMessageFile] = useState({
    messageElementArray: [],
    isDirectory: false,
  });
  ...
  const [editorState, setEditorState] = useState<EditorState>(
    BraftEditor.createEditorState(null, { blockExportFn })
  );
  ...
  const { userId, signature, nickName, faceUrl, role, gender, addPermission } =
    useSelector((state: State.RootState) => state.userInfo);
  const userProfile = {
  ...
  const handleSendMsg = async () => {
    ...
    try {
      ...
      if (messageElementArray[0] && messageElementArray[0].text_elem_content) {
        messageElementArray[0].text_elem_content =
          messageElementArray[0].text_elem_content.substring(
            0,
      ...
      if (messageElementArray.length) {
        //解决换行多次发送问题  -- zwc
        let textElement = messageElementArray.filter(
          (item) => item.elem_type == 0
        );
        if (textElement.length > 0) {
          let outerlement = messageElementArray.filter(
            (item) => item.elem_type != 0
          );
          let obj: any = {
            elem_type: textElement[0].elem_type,
            text_elem_content: textElement
              .map((item) => item.text_elem_content)
              .join("\n"),
          };
  ...
  const filePathAdapter = async (file) => {
    ...
    if (templateFile.path === "") {
      const formatedFile =
        file instanceof File && (await fileReaderAsBuffer(file));
  ...
  const setFile = async (
    ...
    if (file) {
      ...
      if (
        SUPPORT_IMAGE_TYPE.find((v) => type.includes(v)) ||
        type == "image/jpeg"
      ) {
        if (fileSize > 28 * 1024 * 1024)
          return message.error({ content: "image size can not exceed 28m" });
        const imgUrl =
          file instanceof File
            ? await fileImgToBase64Url(file)
            : bufferToBase64Url(file.fileContent, type);
        // @ts-ignore
        setEditorState((preEditorState) =>
          ContentUtils.insertAtomicBlock(preEditorState, "block-image", true, {
            name: file.name,
            path: file.path,
            size: file.size,
            base64URL: imgUrl,
        ...
        if (fileSize > 100 * 1024 * 1024)
          return message.error({ content: "video size can not exceed 100m" });
        ...
        // @ts-ignore
        setEditorState((preEditorState) =>
          ContentUtils.insertAtomicBlock(preEditorState, "block-video", true, {
            name: file.name,
            path: file.path,
            size: file.size,
          })
        ...
        if (fileSize > 100 * 1024 * 1024)
          return message.error({ content: "file size can not exceed 100m" });
        // @ts-ignore
        setEditorState((preEditorState) =>
          ContentUtils.insertAtomicBlock(preEditorState, "block-file", true, {
            name: file.name,
            path: file.path,
            size: file.size,
          })
  ...
  const onAtPopupCallback = (userId: string, userName: string) => {
    ...
    if (userId) {
      ...
      const justHaveAt =
        editorState
          .toText()
          .substring(editorState.toText().lastIndexOf("@")) === "@";
      if (!justHaveAt) {
        const { isUnDoExpected, newEditorState, differenceText } =
          clearAtText(editorState);
        if (isUnDoExpected) {
          setEditorState(ContentUtils.insertText(newEditorState, `${atText} `));
        } else {
          setEditorState(
            ContentUtils.insertText(newEditorState, `@${atText} `)
          );
        }
      } else {
  ...
  const menu = (close) => (
    <List type="option" style={{ width: "200px", background: "#ffffff" }}>
      <List.Item
        onClick={() => changeSendShotcut("1")}
        style={{ display: "flex" }}
      >
        {sendType == "1" ? (
          <img className="chooseImg" src={chooseImg}></img>
        ) : (
  ...
  useEffect(() => {
    ...
    setSendType(initVal);
    setShotKeyTip(
      sendType == "1" ? " 按Ctrl+Enter键发送消息" : "按Enter键发送消息"
    );
    ...
    const screenShotUrlListiner = (event, { data, url }) => {
      ...
      if (data.length == 0) {
        ...
        ipcRenderer.send("removeScreeen");
      } else {
        const file = new File([data], new Date().getTime() + "screenShot.png", {
          type: "image/jpeg",
        });
        // const fileObj = {
  ...
  return (
    <div
      className={`message-input ${shutUpStyle} ${dragEnterStyle}`}
    ...
    >
      {atPopup && (
        <AtPopup
          callback={(userId, name) => onAtPopupCallback(userId, name)}
          atUserNameInput={atUserNameInput}
          group_id={convId}
        />
      )}
      <div className="message-input__feature-area">
        ...
        {FEATURE_LIST[convType].map(({ id, content }) => (
          <span
            key={id}
            title={content}
            className={`message-input__feature-area--icon ${id} ${
              activeFeature === id ? "is-active" : ""
            }`}
      ...
      {isRecordPopup && (
        <RecordPopup
          onSend={handleRecordPopupCallback}
          onCancel={() => setRecordPopup(false)}
        />
      )}
    </div>
  );
