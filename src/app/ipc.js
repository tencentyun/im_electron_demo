...
const setPath = () => {
  const ffprobePath = app.isPackaged
    ? path.resolve(
        ...
        `extraResources/${os.platform()}/${os.arch()}/ffprobe`
      )
    : path.resolve(
        process.cwd(),
        `extraResources/${os.platform()}/${os.arch()}/ffprobe`
      );
  const formateFfmpegPath = app.isPackaged
    ? path.resolve(
        process.resourcesPath,
        `extraResources/${os.platform()}-${os.arch()}/ffmpeg`
      )
    : path.resolve(
        process.cwd(),
        `extraResources/${os.platform()}-${os.arch()}/ffmpeg`
  ...
  settingModle() {
    ...
    try {
      ...
      const fileData = fs.readFileSync(
        SETTING_FILES_ITEM + "/setting.txt",
        "utf-8"
      );
    ...
    try {
      ...
      if (!fs.existsSync(file_path)) {
        ...
        if (!originDownloadKey) {
          ...
          // 已存在
          console.log(
            path.resolve(downloadDicPath, file_name),
            "已存在，不下载"
          );
        }
      }
    ...
    const fileStream = fs
      ...
      .on("finish", function () {
        try {
          ...
          if (fileid) {
            try {
              that.win.webContents.send(fileid, {
                isFinish: true,
                percentage: 100,
              });
            } catch (err) {
      ...
      try {
        FFmpeg(filePath)
          .on("end", async (err, info) => {
            const { width, height, type, size } = await this._getImageInfo(
              screenshotPath
            );
            resolve({
              videoDuration,
